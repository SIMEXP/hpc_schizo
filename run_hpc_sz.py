...
def run_acc(dyn, nSubtypes_fmri, nSubtypes_stage2, file_name):
    ...
    for train, test in skf.split(np.zeros(len(y)), y):
        sbp_cv = sbp.SBP(verbose=False, gamma=1., gamma_auto_adjust=True, thresh_ratio=0.03, min_gamma=0.85, nSubtypes=nSubtypes_fmri,
...
for dyn in [False]:
    for nSubtypes in [3]:
        for nSubtypes_s2 in [3]:
            file_name = '/home/cdansereau/data/sbp/sz_6sites_preproc/results_sz_fmri/sbp_res_cam20_split80'
